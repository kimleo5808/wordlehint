...
export interface LetterFaq {
  question: string;
  /** May contain {N}, {COMMON}, {ANSWERED} placeholders. */
  answer: string;
}

export interface LetterContent {
  letter: string; // single uppercase letter
  path: string; // e.g. "/5-letter-words/starting-with-a"
  lastUpdated: string; // YYYY-MM-DD
  heroSubhead: string;
  topOpener: string; // shown in the third stat card
  /** Extra intro paragraph(s) after the auto-generated stat sentence. */
  introExtra?: ReactNode;
  openersIntro: string;
  openers: Opener[];
  /** Letter-specific solving tips (each rendered as a paragraph). */
  strategyParagraphs: ReactNode[];
  faq: LetterFaq[];
}

const ENDING_PATTERNS = ["E", "T", "Y", "N", "R", "D"];
...
/** Shared template for every "5 letter words starting with X" landing page. */
export function LetterListPage({ content }: { content: LetterContent }) {
  const L = content.letter.toUpperCase();
  const stats = letterStats(L);
  const groups = groupBySecondLetter(L);
  const answered = answeredWordsForLetter(L);
  const faq = content.faq.map((f) => ({
    question: f.question,
    answer: fillFaq(f.answer, stats.total, stats.common, stats.answered),
  }));
  const maxSecond = stats.topSecondLetters[0]?.count ?? 1;
  ...
  const jumpLinks = [
    ...
    { href: "#by-ending", label: "By ending letter" },
    { href: "#faq", label: "FAQ" },
  ];
    ...
    <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: `Starting With ${L}`, url: `${BASE_URL}${content.path}` },
        ])}
      />
      <JsonLd data={faqPageSchema(faq)} />
      <JsonLd
        data={itemListSchema(
          `Best 5-letter Wordle opening words starting with ${L}`,
      ...
      >
        ...
        <span aria-hidden>/</span>
        <span className="text-foreground">Starting with {L}</span>
      </nav>
      ...
      <header className="relative overflow-hidden rounded-3xl border border-border bg-card px-6 py-10 sm:px-10">
        ...
        <div className="relative z-10 max-w-2xl animate-fade-in-up">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3 sm:gap-4">
            <StatCard value={stats.total.toLocaleString()} label={`words starting with ${L}`} />
            <StatCard value={stats.common.toLocaleString()} label="common answer words" />
            <StatCard value={content.topOpener} label={`top-rated ${L} opener`} />
          </div>
      ...
      <section className="mt-10 max-w-3xl space-y-4 text-[15px] leading-relaxed text-muted-foreground">
        <p>
          ...
          <strong className="text-foreground">
            {stats.total.toLocaleString()} five-letter words starting with {L}
          </strong>{" "}
          in the Wordle dictionary, and{" "}
          <strong className="text-foreground">{stats.common.toLocaleString()}</strong>{" "}
          of them are common answer-pool words. The strongest Wordle opener here
          is <strong className="text-foreground">{content.topOpener}</strong>.
        </p>
        {content.introExtra}
      </section>
      ...
      <section className="mt-12">
        <SectionHeading id="best-openers" title={`Best Wordle Opening Words Starting With ${L}`} />
      ...
      <section className="mt-12">
        <SectionHeading
          id="full-list"
          title={`Complete List of 5-Letter Words Starting With ${L}`}
          count={`${stats.total.toLocaleString()} words`}
        />
        <p className="mt-3 max-w-3xl text-[15px] leading-relaxed text-muted-foreground">
          Every valid five-letter {L} word, grouped by its second letter. Common
          answer-pool words are listed first; tap any highlighted word for its
          definition. A{" "}
      ...
      {/* PAST ANSWERS */}
      <section className="mt-12">
        <SectionHeading id="past-answers" title={`${L} Words That Have Been Wordle Answers`} />
        <div className="mt-5 flex flex-col gap-6 rounded-2xl border-2 border-wordle-present/40 bg-wordle-present/5 p-6 sm:flex-row sm:items-start">
          ...
          <div className="min-w-0">
            <p className="font-heading text-2xl font-bold text-foreground">
              {stats.answered} {L}-words have been official Wordle answers
            </p>
      ...
      {/* STRATEGY */}
      <section className="mt-12">
        <SectionHeading id="strategy" title={`How to Solve Wordle When the Word Starts With ${L}`} />
        <div className="mt-5 grid gap-8 lg:grid-cols-[1fr_300px]">
          <div className="space-y-4 text-[15px] leading-relaxed text-muted-foreground">
            ...
            <h3 className="pt-2 font-heading text-lg font-bold text-foreground">
              Most common second letters after {L}
            </h3>
            <ul className="space-y-2">
              {stats.topSecondLetters.map((s) => (
                <li key={s.letter} className="flex items-center gap-3">
                  ...
                  <span className="h-2 flex-1 overflow-hidden rounded-full bg-muted">
                    <span
                      ...
                      style={{ width: `${(s.count / maxSecond) * 100}%` }}
                    />
                  </span>
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-2xl border border-border bg-card p-5">
              <img
                src="/illustrations/solve-strategy-board.svg"
                alt={`Mini Wordle board showing an opener strategy with a green ${L} in the first column`}
                width={260}
                height={312}
      ...
      {/* BY ENDING LETTER */}
      <section className="mt-12">
        <SectionHeading id="by-ending" title={`5-Letter ${L} Words by Ending Letter`} />
        <p className="mt-3 max-w-3xl text-[15px] leading-relaxed text-muted-foreground">
          Already know the last letter too? These are the most common answer
          words that fit each {L} _ _ _ pattern.
        </p>
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {ENDING_PATTERNS.map((end) => {
            const words = commonEndingWith(L, end, 8);
            return (
              <div key={end} className="rounded-2xl border border-border bg-card p-4">
                <TilePattern filled={{ 0: L, 4: end }} size="xs" />
